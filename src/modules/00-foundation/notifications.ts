/**
 * Foundation notifications — sender-only Telegraf untuk modul yang BUKAN
 * 10-telegram-bot tapi butuh push outbound (cron-based optimizer,
 * approval-queue executor, anomaly-alerts, dll).
 *
 * Sebelumnya di `10-telegram-bot/notifications.ts`. Dipindah ke
 * 00-foundation April 2026 untuk break circular dependency
 * 11→10, 12→10. Bot polling instance tetap di
 * `10-telegram-bot/bot.ts` — itu beda concern (incoming command).
 *
 * Sender-only: hanya outbound, tidak terima update. Singleton lazy.
 */
